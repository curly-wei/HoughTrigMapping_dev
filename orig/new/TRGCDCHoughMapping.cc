...
void superLayer(const unsigned id, std::ofstream& outcM, std::ofstream& outcP);
...
constexpr double minY = 1.823908740944321;
constexpr double maxY = 3.204119982655926;
//cell number
constexpr unsigned nX = 160;
...
const std::string result_file_name_M_CC("HoughMappingMinus.cc");
const std::string result_file_name_P_CC("HoughMappingPlus.cc");
const std::string result_file_name_data_M("minus_total.dat");
const std::string result_file_name_data_P("plus_total.dat");

//...C++ for TSIM...
int main(int, char**) {
  fs::create_directories(resut_subdir_name);
  
  auto fncM = resut_subdir_name + "/" + result_file_name_M_CC;
  auto fncP = resut_subdir_name + "/" + result_file_name_P_CC;
  auto outputM_name = resut_subdir_name + "/" + result_file_name_data_M;
  auto outputP_name = resut_subdir_name + "/" + result_file_name_data_P;
  ofstream outcM(fncM);
  ofstream outcP(fncP);
  ofstream outputM(outputM_name);
  ofstream outputP(outputP_name);
...
///////////////////////////////////////////////////HP to TS
  ...
  constexpr double r0 = minY;
  constexpr double phi0 = 0;
  constexpr double dr = (maxY - minY) / nY;
  constexpr double dphi = PI2 / nX;
  double r1;
  double r2;
  double phi1;
  double phi2;
  ...
  const string vhM = resut_subdir_name + "/" + "UT3_0_SL" + to_string(id * 2) + ".vhd";
  const string vhP = resut_subdir_name + "/" + "UT3_0_SL" + to_string(id * 2) + "_p.vhd";
